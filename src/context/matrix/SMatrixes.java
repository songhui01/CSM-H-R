...
public class SMatrixes {
	...
	private Vector<Vector<Integer>>  cssmMatrix= new Vector<Vector<Integer>>();
	...
	public Vector addState(int preState){ 
		//handle initial
		if(cssmMatrix.size()==0||preState==-1){
		...
		Vector<Integer> r=new Vector<>();
		for(int i=0;i<cssmMatrix.size();i++){
            r.add(0);
        }
		cssmMatrix.add(r);
		
		//add column to all rows
		for(int j=0;j<cssmMatrix.size();j++){
			r=cssmMatrix.get(j);
			if(j==preState){
				cssmMatrix.get(j).add(1);
			} else{
				cssmMatrix.get(j).add(0);
			}
	    }
	...
	public Vector updateHmStates(int row, int column){
		cssmMatrix.get(row).set(column, cssmMatrix.get(row).get(column).intValue()+1);
		return cssmMatrix;
	}
	...
	public Vector<Vector<Integer>> getCssmMatrix() {
		return cssmMatrix;
	}

	public void setCssmMatrix(Vector<Vector<Integer>> cssmMatrix) {
		this.cssmMatrix = cssmMatrix;
	}
